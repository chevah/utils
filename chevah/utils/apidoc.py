...
"""
    ...
    Parses a directory tree looking for Python modules and packages and
...
def get_module_path(module_name):
    ...
    if len(module_parts) > 1:
        rootpath = os.path.join(rootpath, *module_parts[1:])
    return rootpath


def recurse_tree(rootpath_or_module, excludes, opts, use_module):
    """
    ...
    if use_module:
        rootpath = get_module_path(rootpath_or_module)
    else:
        rootpath = os.path.abspath(rootpath_or_module)

    if use_module:
        package_name = rootpath_or_module
    else:
    ...
    for root, subs, files in tree:
        ...
        # could add check for windows hidden files
...
Note: By default this script will not overwrite already created files.""")
    ...
    recurse_tree(rootpath, excludes, opts, use_module)
